...
export default function Marché() {
  ...
  const ilikePattern = toIlikePattern(searchTerm);

  const { data: items = [], isLoading, isError } = useQuery({
    queryKey: ["marketplace-items", searchTerm, category, condition, location, maxPoints],
    queryFn: async () => {
      let query = supabase
        .from("items")
        .select("id,title,description,image_url,points,location,created_at,status,user_id,category,condition")
        .eq("status", "available")
        .order("created_at", { ascending: false });

      if (ilikePattern) {
        query = query.or(
          `title.ilike.${ilikePattern},description.ilike.${ilikePattern},location.ilike.${ilikePattern}`
        );
      }
      ...
      if (maxPoints !== "all") query = query.lte("points", parseInt(maxPoints, 10));
    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-card border rounded-2xl p-4 grid grid-cols-1 md:grid-cols-12 gap-3">
        ...
        <Select value={category} onValueChange={setCategory}>
          <SelectTrigger className="md:col-span-3 h-10">
        ...
        <Select value={location} onValueChange={setLocalisation}>
          <SelectTrigger className="md:col-span-2 h-10">
      ...
      {isLoading ? (
        ...
        <div className="bg-card border rounded-2xl p-12 text-center text-destructive">
          Impossible de charger les objets. Vérifie ta connexion.
        </div>
      ) : filtered.length === 0 ? (
        <div className="bg-card border rounded-2xl p-12 text-center text-muted-foreground">
          {searchTerm
            ? `Aucun objet ne correspond à « ${searchTerm} ».`
